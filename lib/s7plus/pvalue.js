...
/**
 * Guard against corrupt/truncated PDUs: a decoded length field must never
 * exceed the bytes actually available. Without this check a garbage
 * length (e.g. from a mis-framed PDU) drives the element loops for
 * millions of iterations — fully synchronous on the event loop — which
 * freezes the whole Node-RED process (timers, dashboard, even SIGINT).
 */
function assertDecodable(buf, need, what) {
    if (need > buf.remaining()) {
        throw new Error(
            `decode overrun: ${what} needs ${need} byte(s), only ${buf.remaining()} remaining`
        );
    }
}
...
function deserialize(buf, disableVlq = false) {
    ...
    switch (datatype) {
        ...
        case Datatype.Blob: {
            ...
            assertDecodable(buf, len, `blob of size ${len}`);
            const data = buf.readBytes(len);
            return new ValueBlob(0, data || Buffer.alloc(0), flags);
        }
        case Datatype.WString: {
            const len = (disableVlq ? S7p.decodeUInt32(buf) : S7p.decodeUInt32Vlq(buf)).v;
            assertDecodable(buf, len, `wstring of size ${len}`);
            const data = buf.readBytes(len);
...
function _deserializeArray(buf, datatype, flags, disableVlq) {
    ...
    // Every element consumes at least one byte on the wire, so the
    // element count can never exceed the remaining bytes.
    assertDecodable(buf, len, `array[${len}] of datatype 0x${datatype.toString(16)}`);
    const arr = [];
    switch (datatype) {
        case Datatype.Bool:
        ...
        case Datatype.Blob:
            for (let i = 0; i < len; i++) {
                ...
                assertDecodable(buf, sz, `blob[${i}] of size ${sz}`);
        ...
        case Datatype.WString:
            for (let i = 0; i < len; i++) {
                const sz = (disableVlq ? S7p.decodeUInt32(buf) : S7p.decodeUInt32Vlq(buf)).v;
                assertDecodable(buf, sz, `wstring[${i}] of size ${sz}`);
...
function _deserializeSparseArray(buf, datatype, flags, disableVlq) {
    ...
    while (key > 0) {
        assertDecodable(buf, 1, `sparse array entry (key ${key})`);
        let v;
        switch (datatype) {
            case Datatype.UDInt:
            ...
            case Datatype.Blob: {
                const sz = (disableVlq ? S7p.decodeUInt32(buf) : S7p.decodeUInt32Vlq(buf)).v;
                assertDecodable(buf, sz, `sparse blob of size ${sz}`);
                v = buf.readBytes(sz) || Buffer.alloc(0); break;
            }
            case Datatype.WString: {
                const sz = (disableVlq ? S7p.decodeUInt32(buf) : S7p.decodeUInt32Vlq(buf)).v;
                assertDecodable(buf, sz, `sparse wstring of size ${sz}`);
                const data = buf.readBytes(sz);
...
function _deserializeStruct(buf, flags, disableVlq) {
    ...
    if (isPackedStructId(id)) {
        ...
        assertDecodable(buf, ec.v, `packed struct blob of size ${ec.v}`);
        const data = buf.readBytes(ec.v);
        if (!data) throw new Error(`decode overrun: packed struct blob of size ${ec.v}`);
        stru.addStructElement(id, new ValueBlob(0, data));
    } else {
        let key = disableVlq ? S7p.decodeUInt32(buf) : S7p.decodeUInt32Vlq(buf);
        while (key.v > 0) {
            assertDecodable(buf, 1, `struct element (key ${key.v})`);
