...
const { RESOLVE_YIELD_EVERY, yieldToEventLoop } = require('../cooperative');
...
async function findChildByLabel(client, parentId, label) {
    ...
    for (const child of children) {
        if (child.nodeKind === 'arrpage') {
            // Large arrays expand into thousands of pages; each scan is a
            // cache-hit microtask. Yield per page so the loop stays alive.
...
async function resolveSymbolicBatch(client, symbolPaths) {
    ...
    for (let si = 0; si < symbolPaths.length; si++) {
        const symbolPath = symbolPaths[si];
        // Cached resolves can finish entirely on the microtask queue.
        // Yield periodically so large batches (e.g. explore → read) do
        // not starve timers and the dashboard websocket.
        if (si > 0 && (si % RESOLVE_YIELD_EVERY) === 0) {
            await yieldToEventLoop();
        }
        try {
            const segments = parseSymbolSegments(symbolPath);
