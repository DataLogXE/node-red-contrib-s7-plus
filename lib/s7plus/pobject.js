...
class PObject {
    ...
    static decodeObjectList(buf) {
        ...
        while (tagId === ElementID.StartOfObject) {
            // Stall guard: a corrupt buffer must never keep this loop
            // spinning without consuming bytes (that would block the
            // event loop synchronously).
            const before = buf.position;
            list.push(PObject.decodeObject(buf, true));
            if (buf.position <= before) {
                throw new Error('decode stalled in object list (truncated or corrupt buffer)');
            }
            tagId = buf.readByte();
            if (tagId === null) break;
            buf.position -= 1;
