...
const { FrameAssembler } = require('./frame-assembler');
...
const { buildSubscriptionObject, parseNotification } = require('./subscription');
const { parsePaom, buildSessionInfoPayload } = require('./session-info');
...
// Large read/write ops are split into lock batches so the user lock is
// released between batches — watchdog ping, other nodes, and less PDU burst.
const MAX_TAGS_PER_LOCK = 500;

// Pause between write lock batches on some S7-1500 CPUs that RST after
// ~2 back-to-back SetMultiVariables PDUs even when each PDU is within
// tagsPerWriteMax. setImmediate alone is not enough.
const WRITE_INTER_BATCH_DELAY_MS = 50;

// Max tags per write lock batch (one PDU). Below tagsPerWriteMax on some
// CPUs that RST when too many full-size write PDUs run in one writeValues.
const WRITE_LOCK_BATCH_CAP = 50;
...
class S7CommPlusClient extends EventEmitter {
    constructor() {
        ...
        // Streaming reassembly of S7+ frames. The transport (especially
        // after the TLS upgrade) delivers raw stream chunks with arbitrary
        // boundaries: multiple frames per chunk, or one frame split across
        // chunks. The assembler handles all of it and only ever hands
        // complete, well-framed PDUs to _dispatchPdu.
        this._frames = new FrameAssembler({
            onPdu: (pdu) => this._dispatchPdu(pdu),
            systemEventVersion: ProtocolVersion.SystemEvent,
        ...
        // Monotonically increasing relation id used when creating subscription
        // objects. Mirrors the reference driver's start value; the PLC stores
        // the subscription under this id and increments expectations per create.
        this._subscriptionRelationId = require('./subscription').SUBSCRIPTION_RELATION_ID_START;

        // Configuration
        this._readTimeout = 10000;
        this._tagsPerReadMax = 20;
        this._tagsPerWriteMax = 20;
        this._limitsCache = null;
        this._freeItemsCache = null;
        this._paomStr = '';
        this._parsedPaom = null;
        this._effectiveProtectionLevel = null;
        this._connectAddress = '';
        this._connectPort = 102;
    ...
    _cachePaomFromServerSession(serverSession) {
        const paom = serverSession && serverSession.getStructElement
            ? serverSession.getStructElement(Ids.LID_SessionVersionSystemPAOMString)
            : null;
        const paomStr = paom ? paom.toJs() : '';
        this._paomStr = paomStr ? String(paomStr) : '';
        this._parsedPaom = parsePaom(this._paomStr);
    }
    ...
    /**
     * Feed one received transport chunk into the streaming frame
     * assembler. The chunk may contain any number of complete or
     * partial S7+ frames (raw TLS stream chunks have no frame
     * alignment). Complete PDUs are handed to `_dispatchPdu` by the
     * assembler's callback.
     */
    _onDataReceived(pdu) {
        // Any inbound bytes are proof the transport is alive — record it
        // BEFORE parsing, so that a slowly-arriving multi-fragment PDU
        // does not look like a dead connection to the watchdog.
        this._lastResponseAt = Date.now();
        this._frames.push(pdu);
    }
     ...
     *
     * S7+ response header layout (after FrameAssembler):
     *   [0]   protocolVersion
    ...
    _dispatchPdu(pdu) {
        ...
        this._lastResponseAt = Date.now();
        const opcode = pdu[1];
        // Notifications (cyclic subscription pushes) carry no sequence number at
        // pdu[8..9], so they must be branched off BEFORE the seq-based lookup.
        if (opcode === Opcode.Notification) {
            try {
                const noti = parseNotification(new BufferStream(pdu));
                if (noti) {
                    log('<- notification', () => [{
                        subscriptionId: noti.subscriptionId,
                        seq: noti.seqNum,
                        values: noti.values.size,
                        errors: noti.errors.size
                    }]);
                    this.emit('notification', noti);
                }
            } catch (e) {
                log('notification parse failed', { msg: e.message });
            }
            return;
        }
    ...
    async connect(address, password = '', username = '', timeoutMs = 10000, port = 102) {
        return this._withLifecycleLock('connect', async () => {
            ...
            this.lastError = 0;
            this._browseState = null;
            this._resetSessionInfoCache();
            this._connectAddress = address || '';
            this._connectPort = port || 102;
            this._lastResponseAt = Date.now();
            ...
            try {
                this._cachePaomFromServerSession(serverSession);
                await this._readSystemLimits();
                await this._legitimate(password, username);
            } catch (e) {
    ...
    async _readSystemLimits() {
        ...
        // Mirrors Thomas CommRessources.ReadMax (LIDs 1000, 1001, 0, 1, 2).
        const lids = [1000, 1001, 0, 1, 2];
        const readlist = lids.map((lid) => {
            const addr = new ItemAddress(Ids.ObjectRoot, Ids.SystemLimits);
            addr.lid.push(lid);
            return addr;
        });
        const { values } = await this._readValuesRaw(readlist);
        const nums = values.map((v) => pvalueToNumber(v));
        if (nums[0] != null) this._tagsPerReadMax = nums[0] || 20;
        if (nums[1] != null) this._tagsPerWriteMax = nums[1] || 20;
        this._limitsCache = {
            tagsPerReadMax: nums[0],
            tagsPerWriteMax: nums[1],
            subscriptionsMax: nums[2],
            attributesMax: nums[3],
            subscriptionMemoryMax: nums[4]
        };
    }

    async _readFreeItems() {
        const lids = [0, 1, 2];
        const readlist = lids.map((lid) => {
            const addr = new ItemAddress(Ids.ObjectRoot, Ids.FreeItems);
            addr.lid.push(lid);
            return addr;
        });
        const { values } = await this._readValuesRaw(readlist);
        const nums = values.map((v) => pvalueToNumber(v));
        this._freeItemsCache = {
            subscriptionsFree: nums[0],
            attributesFree: nums[1],
            subscriptionMemoryFree: nums[2]
        };
        return this._freeItemsCache;
    }

    async _readEffectiveProtectionLevel() {
        ...
        const protRes = await this._requestResponse(
            getProt,
            GetVarSubstreamedResponse.deserializeFromPdu,
            'GetProtection'
        );
        if (!protRes || !protRes.value) throw new Error(errorText(S7Consts.errIsoInvalidPDU));
        const level = pvalueToNumber(protRes.value);
        this._effectiveProtectionLevel = level;
        return level;
    }

    async _legitimate(password, username) {
        void username;
        const paomStr = this._paomStr || '';
        const parsed = this._parsedPaom || parsePaom(paomStr);
        if (!parsed || !parsed.deviceCode) {
            if (password) throw new Error(errorText(S7Consts.errCliFirmwareNotSupported));
            return;
        }

        const accessLevel = await this._readEffectiveProtectionLevel();
    ...
    /**
     * Low-level write — does NOT take the user lock. Used by callers that
     * are already inside a lock (public writeValues).
     */
    async _writeValuesRaw(addressList, writeValues, context = {}) {
        if (!this._connected) throw new Error(errorText(S7Consts.errTCPNotConnected));
        const baseOffset = context.baseOffset || 0;
        const grandTotal = context.grandTotal != null ? context.grandTotal : addressList.length;
        ...
        const chunkMax = Math.max(1, this._tagsPerWriteMax || 20);
        ...
        while (chunkStart < total) {
            ...
            req.inObjectId = 0;
            let count = 0;
            while (count < chunkMax && chunkStart + count < total) {
            ...
            try {
                ...
                const transport = /ECONNRESET|not connected|Client not connected|socket-error/i.test(msg);
                if (transport || !this._connected) {
                    throw new Error(
                        `SetMultiVars chunk failed at offset ${baseOffset + chunkStart}/${grandTotal} ` +
                        `(chunkSize=${count}, tagsPerWriteMax=${chunkMax}): ${msg}`
                    );
                }
                throw e;
            }
    ...
    _writeLockBatchSize() {
        const pduMax = Math.max(1, this._tagsPerWriteMax || 20);
        return Math.min(pduMax, WRITE_LOCK_BATCH_CAP);
    }

    async writeValues(addressList, writeValues) {
        const lockBatch = this._writeLockBatchSize();
        const writeCtx = { grandTotal: addressList.length };
        if (addressList.length <= lockBatch) {
            return this._withUserLock('writeValues', () =>
                this._writeValuesRaw(addressList, writeValues, writeCtx));
        }
        const allErrors = new Array(addressList.length).fill(0n);
        for (let offset = 0; offset < addressList.length; offset += lockBatch) {
            const sliceLen = Math.min(lockBatch, addressList.length - offset);
            const addrSlice = addressList.slice(offset, offset + sliceLen);
            const valSlice = writeValues.slice(offset, offset + sliceLen);
            const { errors } = await this._withUserLock('writeValues', () =>
                this._writeValuesRaw(addrSlice, valSlice, {
                    baseOffset: offset,
                    grandTotal: addressList.length
                }));
            for (let i = 0; i < sliceLen; i++) {
                allErrors[offset + i] = errors[i];
            }
            if (offset + sliceLen < addressList.length) {
                await new Promise(resolve => setImmediate(resolve));
                if (WRITE_INTER_BATCH_DELAY_MS > 0) {
                    await new Promise(resolve => setTimeout(resolve, WRITE_INTER_BATCH_DELAY_MS));
                }
            }
        }
    ...
    /**
     * Batch-resolve multiple symbolic PLC paths via lazy browse-tree walk.
     * Not used by Node-RED read/write/subscribe nodes — those resolve via
     * scoped browseFull + _crcCache (see s7complus-endpoint resolveSymbolsBatch).
     * Kept for live diagnostic scripts and direct client callers.
    ...
    async browseFull(options = {}) {
        return this._withUserLock('browseFull', async () => {
            ...
            const flat = await buildFlatSymbolList(exploreData, typeInfoObjects, flatOptions);
            const meta = {
    ...
    /**
     * Session and PLC diagnostic snapshot. Cached PAOM/session IDs come from
     * connect; limits, free items and protection level are refreshed when
     * refreshLimits is true (default).
     */
    async getSessionInfo({ refreshLimits = true } = {}) {
        return this._withUserLock('getSessionInfo', async () => {
            if (!this._connected) throw new Error(errorText(S7Consts.errTCPNotConnected));

            if (refreshLimits) {
                await this._readSystemLimits();
                await this._readFreeItems();
                await this._readEffectiveProtectionLevel();
            }

            return buildSessionInfoPayload(
                {
                    paom: this._paomStr || null,
                    parsedPaom: this._parsedPaom,
                    sessionId: this._sessionId,
                    sessionId2: this._sessionId2,
                    limits: this._limitsCache || {},
                    freeItems: this._freeItemsCache || {},
                    connected: this._connected,
                    lastResponseAt: this._lastResponseAt
                },
                {
                    address: this._connectAddress || null,
                    port: this._connectPort || 102,
                    timeoutMs: this._readTimeout
                }
            );
        });
    }
    ...
    _nextSubscriptionRelationId() {
        const id = this._subscriptionRelationId >>> 0;
        this._subscriptionRelationId = (this._subscriptionRelationId + 1) >>> 0;
        return id;
    }

    /**
     * Create a data-change subscription on the PLC. The PLC then pushes
     * Notification PDUs (handled in _dispatchPdu -> emit('notification')).
     * Mirrors SubscriptionCreate in the reference driver.
     *
     * @param {object} opts
     * @param {Array<{name, address (ItemAddress), datatype?}>} opts.items
     * @param {number} [opts.cycleMs]
     * @param {number} [opts.routeMode]
     * @param {number} [opts.creditLimit]
     * @returns {Promise<{subscriptionObjectId: number, relationId: number, refToName: Map}>}
     */
    async createSubscription(opts) {
        return this._withUserLock('createSubscription', async () => {
            if (!this._connected) throw new Error(errorText(S7Consts.errTCPNotConnected));
            const items = opts && opts.items ? opts.items : [];
            if (!items.length) throw new Error('createSubscription: no items');
            const relationId = this._nextSubscriptionRelationId();
            const { object, refToName } = buildSubscriptionObject(items, {
            ...
            const req = new CreateObjectRequest(ProtocolVersion.V2, 0, true);
            req.transportFlags = 0x34;
            req.requestId = this._sessionId2;
            req.requestValue = new pvalue.ValueUDInt(0);
            req.requestObject = object;

            const res = await this._requestResponse(req, CreateObjectResponse.deserializeFromPdu, 'CreateSubscription');
            if (!res || !res.objectIds || !res.objectIds.length) {
                throw new Error(errorText(S7Consts.errIsoInvalidPDU));
            }
            if (res.returnValue && Number(res.returnValue) !== 0) {
                // The PLC may still have created the object; the caller should
                // delete it. We surface the raw return value for diagnostics.
                throw new Error(`Subscription create failed (returnValue=0x${Number(res.returnValue).toString(16)})`);
            }
            const subscriptionObjectId = res.objectIds[0];
            log('subscription created', { subscriptionObjectId, relationId, items: items.length });
            return { subscriptionObjectId, relationId, refToName };
        });
    }

    /**
     * Delete a previously created subscription object on the PLC.
     * @param {number} subscriptionObjectId - the id returned by createSubscription
     */
    async deleteSubscription(subscriptionObjectId) {
        return this._withUserLock('deleteSubscription', async () => {
            if (!this._connected) throw new Error(errorText(S7Consts.errTCPNotConnected));
            const del = new DeleteObjectRequest(ProtocolVersion.V2);
            del.deleteObjectId = subscriptionObjectId >>> 0;
            del.withIntegrityId = true;
            await this._requestResponse(del, () => ({}), 'DeleteSubscription');
            log('subscription deleted', { subscriptionObjectId });
        });
    }
